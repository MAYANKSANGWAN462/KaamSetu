...
const SectionCard = ({ title, children, step }) => (
  <div className="bg-white dark:bg-white/[0.04] rounded-3xl border border-[#e8dfd0] dark:border-white/8 p-7 shadow-sm">
    <div className="flex items-center gap-3 mb-6">
      ...
      <h2 className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#9c8a78]">{title}</h2>
    </div>
    {children}
  </div>
)
...
const WorkerSetup = () => {
  ...
  useEffect(() => {
    ...
    setSkillSuggestions(
      (SKILL_LIST || []).filter(s => s.toLowerCase().includes(q) && !form.skills.includes(s)).slice(0, 6)
    )
  }, [skillInput, form.skills])

  const addSkill = (skill) => {
    if (!form.skills.includes(skill)) setForm(p => ({ ...p, skills: [...p.skills, skill] }))
  ...
  const useMyLocation = () => {
    if (geo.latitude && geo.longitude) {
      setForm(p => ({ ...p, lat: geo.latitude, lng: geo.longitude, locationAddress: geo.manualLocation || 'Current location' }))
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.wageAmount || Number(form.wageAmount) <= 0) { toast.error('Please enter a valid wage'); return }
    ...
    <div className="min-h-screen bg-[#faf7f2] dark:bg-[#0e0d0b] pt-24 pb-12">
      <div className="max-w-2xl mx-auto px-4">
        ...
        <motion.div {...stagger(0)} className="mb-8">
          ...
          <h1 className="text-3xl font-black text-gray-900 dark:text-white">Set Up Your Profile</h1>
          <p className="text-sm text-[#9c8a78] mt-1">Let hirers find you for the right jobs.</p>
        </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <SectionCard title="Your Work Category" step="1">
              ...
              <div className="flex flex-wrap gap-2">
                {categories.map(cat => {
                  ...
                  return (
                    <button key={val} type="button" onClick={() => setForm(p => ({ ...p, category: val }))}
          ...
          <motion.div {...stagger(2)}>
            <SectionCard title="Your Skills" step="2">
              <Label>Add Skills (select from list)</Label>
                ...
                <AnimatePresence>
                  {skillSuggestions.length > 0 && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-[#1a1208] border border-[#e8dfd0] dark:border-white/10 rounded-2xl shadow-xl z-10 overflow-hidden"
                    >
                      {skillSuggestions.map(skill => (
                        <button key={skill} type="button" onClick={() => addSkill(skill)}
                          className="w-full text-left px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-[#faf7f2] dark:hover:bg-white/[0.06] transition-colors duration-150 first:rounded-t-2xl last:rounded-b-2xl">
                          {skill}
                        </button>
                      ))}
                    </motion.div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-[#b8a898] font-bold text-sm pointer-events-none">₹</div>
                    <input type="number" min="0" value={form.wageAmount}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {(wageUnits.length > 0 ? wageUnits : ['hourly', 'daily', 'per job']).map(unit => (
                      <button key={unit} type="button" onClick={() => setForm(p => ({ ...p, wageUnit: unit }))}
          ...
          <motion.div {...stagger(4)}>
            <SectionCard title="Availability" step="4">
              <div className="flex gap-3">
                {[{ val: true, label: '✅ Available Now' }, { val: false, label: '⏸ Not Available' }].map(({ val, label }) => (
                  <button key={String(val)} type="button" onClick={() => setForm(p => ({ ...p, isAvailable: val }))}
          ...
          <motion.div {...stagger(6)}>
            <SectionCard title="About You" step="6">
              ...
              <div className="relative">
                ...
                <p className="text-[10px] text-[#b8a898] dark:text-gray-600 text-right mt-1">{form.bio.length}/300</p>
              </div>
            </SectionCard>
          </motion.div>
