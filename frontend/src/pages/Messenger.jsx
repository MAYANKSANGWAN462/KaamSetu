...
const Avatar = ({ photo, name, size = 'md' }) => {
  ...
  return photo ? (
    <img src={photo} alt={name} className={`${s} rounded-2xl object-cover border-2 border-[#e6e8ec] dark:border-white/10`} />
  ) : (
...
const Messenger = () => {
  ...
  /* ── render ── */
  return (
    <div className="h-[calc(100dvh-4rem)] bg-[#f6f7f9] dark:bg-[#0b0e14] flex overflow-hidden">
      ...
      <aside className={`w-full md:w-80 lg:w-96 flex-shrink-0 border-r border-[#e6e8ec] dark:border-white/8 bg-white dark:bg-white/[0.02] flex flex-col ${conversationId ? 'hidden md:flex' : 'flex'}`}>
        <div className="px-5 pt-5 pb-3 border-b border-[#e6e8ec] dark:border-white/8">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-2xl bg-[#f6f7f9] dark:bg-white/[0.04] border border-[#e6e8ec] dark:border-white/10 pl-10 pr-4 py-2.5 text-sm text-gray-800 dark:text-gray-200 placeholder:text-[#94a3b8] outline-none focus:border-[#c8933a]/60 transition-all duration-300"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingConvs ? (
              ...
              {[...Array(6)].map((_, i) => (
                <div key={i} className="flex items-center gap-3 px-2 py-3 animate-pulse">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="h-3.5 w-1/3 bg-[#e6e8ec] dark:bg-white/10 rounded-lg" />
                    <div className="h-3 w-2/3 bg-[#e6e8ec] dark:bg-white/10 rounded-lg" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center justify-center h-full text-center px-8">
              <div className="w-16 h-16 rounded-3xl bg-gradient-to-br from-[#f6f7f9] to-[#eef0f3] dark:from-white/[0.06] dark:to-white/[0.03] border border-[#e6e8ec] dark:border-white/10 flex items-center justify-center text-2xl mb-3">💬</div>
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition-all duration-200 ${
                      isActive
                        ...
                        : conv.unreadCount > 0
                          ...
                          : 'hover:bg-[#f6f7f9] dark:hover:bg-white/[0.03]'
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-0.5">
                        ...
                        <span className="text-[10px] text-[#94a3b8] flex-shrink-0 ml-2">{formatListTime(conv.lastMessageTime)}</span>
                      </div>
                      <div className="flex items-center justify-between gap-2">
                        <p className={`text-xs truncate ${conv.unreadCount > 0 ? 'text-gray-700 dark:text-gray-300 font-semibold' : 'text-[#6b7280]'}`}>
      ...
      <section className={`flex-1 flex-col min-w-0 ${conversationId ? 'flex' : 'hidden md:flex'}`}>
        {!conversationId ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
            <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-[#f6f7f9] to-[#eef0f3] dark:from-white/[0.06] dark:to-white/[0.03] border border-[#e6e8ec] dark:border-white/10 flex items-center justify-center text-4xl mb-5 shadow-sm">💬</div>
            <p className="font-black text-xl text-gray-900 dark:text-white mb-1">Your messages</p>
            <p className="text-sm text-[#6b7280] max-w-xs">Select a conversation to start chatting.</p>
          </div>
        ) : (
          <>
            {/* chat header */}
            <div className="flex-shrink-0 bg-white/90 dark:bg-[#0b0e14]/90 backdrop-blur-xl border-b border-[#e6e8ec] dark:border-white/8 px-4 py-3 flex items-center gap-3">
              <button
                onClick={() => navigate('/messages')}
                className="md:hidden w-9 h-9 rounded-xl border border-[#e6e8ec] dark:border-white/10 flex items-center justify-center text-[#6b7280] hover:text-[#c8933a]"
              >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-5 space-y-1">
              {loadingMsgs ? (
                <div className="space-y-4">
                  {[false, true, false, true, false].map((me, i) => (
                    <div key={i} className={`flex ${me ? 'justify-end' : 'justify-start'}`}>
                      <div className={`h-10 rounded-2xl animate-pulse ${me ? 'bg-[#c8933a]/20' : 'bg-[#e6e8ec] dark:bg-white/10'}`} style={{ width: `${120 + i * 26}px` }} />
                    </div>
                  ))}
                </div>
              ) : messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  ...
                  <p className="text-sm text-[#6b7280]">Send a message to start the conversation.</p>
                </div>
              ) : (
                grouped.map(([date, msgs]) => (
                  <div key={date}>
                    <div className="flex items-center gap-3 my-4">
                      <div className="flex-1 h-px bg-[#e6e8ec] dark:bg-white/8" />
                      <span className="text-[10px] font-bold tracking-[0.1em] uppercase text-[#94a3b8] px-3 py-1 rounded-full bg-[#f6f7f9] dark:bg-white/[0.04] border border-[#e6e8ec] dark:border-white/8">{date}</span>
                      <div className="flex-1 h-px bg-[#e6e8ec] dark:bg-white/8" />
                    </div>
                            ...
                            <div className="max-w-[75%] group">
                              <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed break-words ${
                                isMe
                                  ...
                                  : 'bg-white dark:bg-white/[0.07] border border-[#e6e8ec] dark:border-white/10 text-gray-800 dark:text-gray-200 rounded-bl-sm shadow-sm'
                              } ${msg.optimistic ? 'opacity-70' : ''}`}>
                                {contentOf(msg)}
                              </div>
                              <p className={`text-[10px] text-[#94a3b8] mt-1 opacity-0 group-hover:opacity-100 transition-opacity ${isMe ? 'text-right' : 'text-left'}`}>
              ...
              <AnimatePresence>
                {peerTyping && (
                  <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex justify-start">
                    <div className="px-4 py-3 bg-white dark:bg-white/[0.07] border border-[#e6e8ec] dark:border-white/10 rounded-2xl rounded-bl-sm">
            ...
            {/* fixed input */}
            <div className="flex-shrink-0 bg-white/95 dark:bg-[#0b0e14]/95 backdrop-blur-xl border-t border-[#e6e8ec] dark:border-white/8 px-4 py-3">
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  className="flex-1 rounded-2xl bg-[#f6f7f9] dark:bg-white/[0.06] border border-[#e6e8ec] dark:border-white/10 px-4 py-3 text-sm text-gray-800 dark:text-gray-200 placeholder:text-[#94a3b8] outline-none focus:border-[#c8933a]/50 resize-none max-h-32"
